...
float random_vec2_in(vec2 p) { return fract(1e4 * sin(17.0 * p.x + p.y * 0.1) * (0.1 + abs(sin(p.y * 13.0 + p.x)))); }

float bin(vec2 ipos, float n){
    float refragment = mod(n,33554432.);
    for(float i = 0.0; i < 25.0; i++){
        if ( floor(i/3.) == ipos.y && mod(i,3.) == ipos.x ) {
            return step(1.0,mod(refragment,2.));
        }
...
void fragment(){
    ...
    if (digits_st_i.y == 1. &&
        ...
        float pct = random_vec2_in(digits_st_i+floor(crosses_st)+floor(TIME*20.));
